...
class Stderr(object):
  def __init__(self, activation):
    for layer in ['h_conv1', 'h_conv2', 'h_fc1', 'y_conv']:
      ...
      t = np.std(t, axis=1)
      setattr(self, layer, t)

stderr0 = Stderr(train0)

def plot(corr, stderr):
  plt.scatter(corr, stderr)
  plt.xlim(0, 1)
  plt.ylim(0, np.nanmax(stderr)*1.1)

for layer in ['conv1', 'conv2', 'fc1']:
  correlation = getattr(corr, 'h_'+layer)
  stderr = getattr(stderr0, 'h_'+layer)
  plot(correlation, stderr)
  plt.savefig('corr_stderr_'+layer+'.png')
  plt.clf()
